...
std::string lsof_tcp_command() {
    for (const char* path : {"/usr/sbin/lsof", "/usr/bin/lsof", "/opt/homebrew/bin/lsof"}) {
        if (::access(path, X_OK) == 0) {
            return std::string(path) + " -nP -FpcuftnT -iTCP";
        }
    }
    return "lsof -nP -FpcuftnT -iTCP";
}
...
std::vector<ProcessSocketEntry> query_tcp_processes(const PidSearchQuery& query) {
    const std::string output = run_command_capture(lsof_tcp_command());
